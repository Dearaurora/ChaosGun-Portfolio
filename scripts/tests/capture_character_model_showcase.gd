...
const DEFAULT_OUT_PATH := "res://reports/character_model_showcase.png"
const VIEWPORT_SIZE := Vector2i(1536, 800)


func _initialize() -> void:
	if DisplayServer.get_name().to_lower() == "headless":
		push_error("Character model showcase requires a render-capable display driver")
		quit(1)
		return
	...
	if error != OK:
		push_error("Could not save character model showcase to %s" % out_path)
		quit(1)
		return
	print("Saved character model showcase: %s" % ProjectSettings.globalize_path(out_path))
	quit(0)
...
func _build_environment(stage: Node3D) -> void:
	...
	environment.background_color = Color("#7778b9")
...
func _build_floor(stage: Node3D) -> void:
	...
	mesh.size = Vector3(14.0, 0.5, 5.6)
	var material := StandardMaterial3D.new()
	material.albedo_color = Color("#ed935d")
	material.roughness = 0.82
	mesh.material = material
	floor.mesh = mesh
	floor.position = Vector3(0.0, -0.32, 0.0)
...
func _build_camera(stage: Node3D) -> void:
	...
	camera.look_at_from_position(Vector3(0.0, 4.6, -11.5), Vector3(0.0, 1.18, 0.0), Vector3.UP)


func _build_characters(stage: Node3D) -> void:
	var entries := [
		{"x": -4.2, "color": Color("#ef4d53"), "weapon": &"pistol"},
		{"x": 0.0, "color": Color("#75ca3e"), "weapon": &"ak_rifle"},
		{"x": 4.2, "color": Color("#28a8df"), "weapon": &"sniper"},
	]
	for entry in entries:
		var visual := CharacterVisual.new()
		visual.body_color = entry["color"]
		visual.position = Vector3(float(entry["x"]), 0.0, 0.0)
		visual.rotation_degrees.y = 12.0
...
func _build_labels(stage: Node3D) -> void:
	...
	for index in range(3):
		var label := Label.new()
		label.text = ["PISTOL POSE", "RIFLE POSE", "SNIPER POSE"][index]
		label.position = Vector2(120.0 + index * 512.0, 704.0)
