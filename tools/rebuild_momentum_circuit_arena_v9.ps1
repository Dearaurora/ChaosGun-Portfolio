...
$blend = Join-Path $root "assets\source\momentum_circuit_v9\momentum_circuit_foreground_v9.blend"
$glb = Join-Path $root "assets\models\generated\momentum_circuit_v9\momentum_circuit_foreground_v9.glb"

foreach ($required in @($BlenderExe, $builder, $layout)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Momentum Circuit v9 rebuild input is missing: $required"
    }
}

$previousVersion = $env:MC_FOREGROUND_VERSION
try {
    $env:MC_FOREGROUND_VERSION = "9"
    Write-Host "Rebuilding Momentum Circuit v9 visual-only foreground..."
    & $BlenderExe --background --python-exit-code 1 --python $builder
    if ($LASTEXITCODE -ne 0) {
        throw "Momentum Circuit v9 Blender builder failed with exit code $LASTEXITCODE"
    }
}
finally {
...
foreach ($output in @($blend, $glb)) {
    if (-not (Test-Path -LiteralPath $output -PathType Leaf)) {
        throw "Expected Momentum Circuit v9 output is missing: $output"
    }
    if ((Get-Item -LiteralPath $output).Length -le 0) {
        throw "Expected Momentum Circuit v9 output is empty: $output"
    }
}
...
Write-Host "Momentum Circuit v9 foreground rebuild completed."
foreach ($output in @($blend, $glb)) {
