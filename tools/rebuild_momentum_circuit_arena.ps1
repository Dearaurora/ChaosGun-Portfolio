...
$builder = Join-Path $PSScriptRoot "build_momentum_circuit_arena.py"
$layout = Join-Path $root "resources\maps\momentum_circuit_layout_v2.json"
$blend = Join-Path $root "assets\source\momentum_circuit\momentum_circuit_foreground.blend"
$glb = Join-Path $root "assets\models\generated\momentum_circuit\momentum_circuit_foreground.glb"
...
Write-Host "Rebuilding Momentum Circuit visual-only foreground..."
...
Write-Host "Momentum Circuit foreground rebuild completed."
foreach ($output in @($blend, $glb)) {
    $item = Get-Item -LiteralPath $output
